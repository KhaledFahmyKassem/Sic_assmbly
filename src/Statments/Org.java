    ...
    public boolean isValid(String query) {
        ...
        if (ma.matches()) {
            if (!setOperation("+", opran))
                if (!setOperation("-", opran)) {
                    address.add(opran.trim().toLowerCase());
                    if (Character.isAlphabetic(opran.charAt(0)))
...
//            if (x < Controller.startLocation ){
//                System.out.println(query+"this org access after programcount");
//                return false;
//                
//            }
